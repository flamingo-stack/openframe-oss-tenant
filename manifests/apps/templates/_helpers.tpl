...
5. If deployment.saas.enabled → skip "openframe-config" "openframe-api" "openframe-authorization-server" and "ngrok-operator"
...
{{- else if and $oss $ossNgrok (eq $name "ingress-nginx") }}
  true
{{- else if and $saas (or (eq $name "openframe-config") (eq $name "openframe-api") (eq $name "openframe-authorization-server") (eq $name "ngrok-operator")) }}
  true
