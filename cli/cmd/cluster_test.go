...
func TestClusterProviderCreation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			provider, err := createClusterProvider(tt.clusterType, false, os.Stdout)
			if tt.expectErr && err == nil {
				t.Error("expected error but got none")
			}
			if !tt.expectErr && err != nil {
...
func TestFindRepoRoot(t *testing.T) {
	// This test will vary based on the actual file system structure
	// We'll just ensure it doesn't panic and returns something
	_, err := findRepoRoot()
	// Don't assert error since it depends on where the test is run
	// Just ensure the function works
	if err != nil {
		t.Logf("findRepoRoot returned error (may be expected): %v", err)
	}
}
...
func TestClusterCommandFlags(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			outputStr := output.String()
			// Check that basic flags are documented
			if !strings.Contains(outputStr, "--dry-run") || !strings.Contains(outputStr, "--skip-wizard") {
				t.Errorf("expected flag documentation in help, got: %s", outputStr)
			}
		})
	}
}
