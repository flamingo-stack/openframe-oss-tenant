...
func TestRealCommandExecutor_wrapCommandForWindows(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedCommandWindows: "wsl",
			expectedArgsWindows:    []string{"-d", "Ubuntu", "-u", "runner", "sg", "docker", "-c", "k3d cluster create test-cluster"},
			expectedCommandUnix:    "k3d",
			expectedArgsUnix:       []string{"cluster", "create", "test-cluster"},
			description:            "k3d commands should be wrapped with sg docker on Windows for Docker access",
		},
		{
			...
			expectedCommandWindows: "wsl",
			expectedArgsWindows:    []string{"-d", "Ubuntu", "-u", "runner", "sg", "docker", "-c", "k3d cluster create --config /mnt/c/temp/config.yaml"},
			expectedCommandUnix:    "k3d",
		...
		},
		{
			...
			expectedCommandWindows: "wsl",
			expectedArgsWindows:    []string{"-d", "Ubuntu", "-u", "runner", "sg", "docker", "-c", "k3d cluster create 'my cluster' --config '/path with spaces/config.yaml'"},
			expectedCommandUnix:    "k3d",
			expectedArgsUnix:       []string{"cluster", "create", "my cluster", "--config", "/path with spaces/config.yaml"},
			description:            "k3d with spaces in arguments should escape properly",
		},
		{
			name:                   "empty args",
			command:                "kubectl",
