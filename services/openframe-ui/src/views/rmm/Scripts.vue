...
const saveScript = async (scriptData: any) => {
  try {
    ...
    const errorMsg = error.response?.data?.message || error.message || `Failed to ${isEditMode.value ? 'update' : 'create'} script`;
    toastService.showError(errorMsg);
  } finally {
...
const executeScript = async () => {
  ...
  try {
    ...
    const errorMsg = error.response?.data?.message || error.message || 'Failed to run script';
    toastService.showError(errorMsg);
  } finally {
...
const viewScript = async (script: Script) => {
  ...
  try {
    ...
    const errorMsg = error.response?.data?.message || error.message || 'Failed to fetch script details';
    toastService.showError(errorMsg);
    selectedScript.value = {
      ...script,
      script_type: script.script_type
...
const deleteScript = async (script: Script) => {
  try {
    ...
    const errorMsg = error.response?.data?.message || error.message || 'Failed to delete script';
    toastService.showError(errorMsg);
  }
};
