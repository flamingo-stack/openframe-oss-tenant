...
function parseSockJSFrame(raw: string): { type: 'open' | 'data' | 'heartbeat' | 'close'; messages?: CampaignMessage[]; closeInfo?: [number, string] } | null {
...
export function useLiveCampaign(): UseLiveCampaignReturn {
  ...
  const handleCampaignMessage = useCallback((msg: CampaignMessage) => {
  ...
  const startCampaign = useCallback(async (sql: string) => {
    if (!sql.trim()) {
    ...
    try {
      ...
      const ws = new WebSocketManager({
        ...
        onError: () => {
          if (isMountedRef.current) {
            toast({ title: 'WebSocket Error', description: 'Connection error during live campaign', variant: 'destructive' })
          }
        },
        onClose: () => {
