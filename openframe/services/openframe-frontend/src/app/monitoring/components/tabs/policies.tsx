...
export function Policies() {
  ...
  const columns: TableColumn<Policy>[] = useMemo(() => [
    {
      ...
      label: 'Name',
      renderCell: (policy) => (
        <DeviceCardCompact deviceName={policy.name} organization={policy.description} />
      )
    },
    {
      key: 'severity',
  ...
  const rowActions = useCallback((policy: Policy) => [
    {
  ...
  const renderRowActions = useMemo(() => {
    return (policy: Policy) => (
      <MoreActionsMenu items={rowActions(policy)} />
    )
  }, [rowActions])
