...
export function Queries() {
  ...
  const columns: TableColumn<Query>[] = useMemo(() => [
    {
      ...
      label: 'Name',
      renderCell: (query) => (
        <DeviceCardCompact deviceName={query.name} organization={query.description} />
      )
    },
    {
      key: 'frequency',
  ...
  const rowActions = useCallback((query: Query) => [
    {
      label: 'Query Details',
  ...
  const renderRowActions = useMemo(() => {
    return (query: Query) => (
      <MoreActionsMenu items={rowActions(query)} />
    )
  }, [rowActions])

  const handleRowClick = useCallback((query: Query) => {
