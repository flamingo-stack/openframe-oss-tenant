...
import { CardLoader, FormPageContainer, Label, LoadError, NotFoundError, OS_TYPES } from '@flamingo-stack/openframe-frontend-core'
...
export function EditPolicyPage({ policyId }: EditPolicyPageProps) {
  ...
  const { policyDetails, isLoading: isLoadingPolicy, error: policyError } = usePolicyDetails(isExistingPolicy ? numericId : null)
            ...
            <Controller
              ...
              render={({ field }) => {
                const selectedPlatformIds = field.value
                  ? field.value.split(',').map(p => p.trim()).filter(Boolean)
                      .flatMap(platformId => {
