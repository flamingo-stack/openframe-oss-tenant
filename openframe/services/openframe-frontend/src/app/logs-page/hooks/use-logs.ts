...
export function useLogs(activeFilters: LogFilterInput = {}) {
   ...
   */
  const transformLogEntry = useCallback((logEntry: LogEntry): LogEntry => {
  ...
  const fetchLogs = useCallback(
    async (searchTerm: string, filters: LogFilterInput = {}, cursor?: string | null, append: boolean = false) => {
      setIsLoading(true);
      setError(null);
    ...
    [pageSize, toast, appendEdges, setEdges, setError, setPageInfo, transformLogEntry],
  );
...
export function useLogFilters() {
  ...
  const fetchLogFilters = useCallback(
    async (filter?: LogFilterInput) => {
      setIsLoading(true);
      setError(null);
