...
function TagValueAutocomplete({ organizationId, tagKey, values, onChange, showLabel }: TagValueAutocompleteProps) {
...
export function TagRow({
  ...
  const keyOptions: AutocompleteOption[] = useMemo(
    () =>
      (keyData.tagKeySuggestions ?? [])
        .filter(s => !existingKeys.includes(s.key) || s.key === tag.key)
        .map(s => ({ label: s.key, value: s.key })),
    [keyData, existingKeys, tag.key],
  );
    ...
    <div className="flex gap-2 items-end w-full">
      ...
      <div className="flex-1 flex gap-2 items-end">
        ...
        <button
          type="button"
          onClick={onDelete}
          className="flex items-center justify-center h-12 w-12 bg-ods-card border border-ods-border rounded-[6px] text-[var(--ods-attention-red-error)] hover:bg-[#2a2a2a] transition-colors shrink-0"
