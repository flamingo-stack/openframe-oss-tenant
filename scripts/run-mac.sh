...
if [ "$HELP" = true ] && [ ${#RUN_ARGS[@]} -eq 0 ]; then
    cat << EOF
OpenFrame Installation Script
Usage: ./run-mac.sh [-h|--help] [-s|--silent] [COMMAND]
...
Examples:
    ./run-mac.sh -s bootstrap     # Run bootstrap in silent mode
    ./run-mac.sh -s app nginx deploy    # Deploy nginx app in silent mode
    ./run-mac.sh platform              # Setup platform in interactive mode
    ./run-mac.sh --help                # Show this help message
    ./run-mac.sh app --help            # Show run.sh help message
EOF
    exit 0
fi
...
check_docker() {
    write_status_message "Checking Docker Desktop installation..." "\033[36m"
    if ! command -v docker &> /dev/null; then
        write_status_message "Docker Desktop not found. Installing Docker Desktop..." "\033[33m"
        brew install --cask docker
        write_status_message "Docker Desktop installed successfully!" "\033[32m"
        write_status_message "Please start Docker Desktop from your Applications folder." "\033[33m"
        write_status_message "After starting Docker Desktop, run this script again." "\033[33m"
        exit 0
    else
        write_status_message "Docker Desktop is already installed." "\033[32m"
    fi
}
