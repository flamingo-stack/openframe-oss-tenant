...
function trust_ca() {
  ...
  if ! kubectl get secret "$CA_SECRET" -n "$CA_NAMESPACE" -o jsonpath='{.data.ca\.crt}' 2>/dev/null | base64 -d > "$TMP_CA_PATH"; then
  ...
  case "$OS" in
    ...
    Darwin*)
      if security find-certificate -c "$CA_SECRET" -p >/dev/null 2>&1; then
        echo "CA certificate already trusted in macOS Keychain"
      else
