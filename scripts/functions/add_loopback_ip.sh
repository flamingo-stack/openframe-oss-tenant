...
function add_loopback_ip() {
    ...
    if [[ "$OS" == "Linux" ]]; then
        ...
        echo "Adding IP $IP to the loopback interface on macOS"
        # Check if the IP is already added to the loopback interface
        if ifconfig lo0 | grep -q "inet $IP"; then
            echo "IP $IP is already added to the loopback interface."
        else
            # Add the IP address to the loopback interface on macOS using GUI auth dialog
            osascript -e "do shell script \"ifconfig lo0 alias $IP up\" with administrator privileges"
            echo "IP $IP added to the loopback interface."
        fi
