...
function tools_kafka_ui_deploy() {
  helm_repo_ensure kafbat-ui https://kafbat.github.io/helm-charts &&
    ...
    helm upgrade -i kafka-ui kafbat-ui/kafka-ui \
      ...
      --version 1.4.12 \
      -f ${ROOT_REPO_DIR}/manifests/client-tools/kafka-ui/helm/kafka-ui.yaml \
      --wait --timeout 1h
}
...
function tools_mongo_express_deploy() {
  echo "Deploying Mongo Express (UI)" &&
    kubectl -n client-tools apply -f ${ROOT_REPO_DIR}/manifests/client-tools/mongo-express/mongo-express.yaml
}
...
function tools_mongo_express_delete() {
  ...
  kubectl -n client-tools delete -f ${ROOT_REPO_DIR}/manifests/client-tools/mongo-express/mongo-express.yaml
}
