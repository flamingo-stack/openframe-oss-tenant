...
use crate::config::updater_config::{
    CLIENT_SERVICE_FULL_NAME, MIN_BINARY_SIZE_BYTES, SERVICE_START_VERIFY_WAIT_SECS,
};
...
impl ClientUpdateService {
    ...
    async fn run_update(&self, msg: &ClientUpdateMessage) -> Result<()> {
        ...
        info!(
            "Waiting {}s before verifying service state",
        ...
        self.state_service
            .transition(&mut state, UpdaterPhase::Completed)?;
