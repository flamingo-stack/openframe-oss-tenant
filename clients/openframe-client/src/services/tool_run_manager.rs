            ...
            async move {
                ...
                #[cfg(target_os = "macos")]
                {
                    ...
                    match tool.session_type {
                        ...
                        SessionType::Console => {
                            info!(tool_id = %tool.tool_agent_id, "SessionType::Console - skipping on macOS");
                            return;
                        }
                        SessionType::Service => {
                            // Continue to standard spawn below
                ...
                #[cfg(all(unix, not(target_os = "macos")))]
                {
                    ...
                    if tool.session_type == SessionType::Console {
                        info!(tool_id = %tool.tool_agent_id, "SessionType::Console - skipping");
                        return;
                    }
                }

                // Standard spawn for Service session type
