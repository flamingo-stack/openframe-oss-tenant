...
#[tokio::test]
async fn corrupt_db_is_quarantined_and_store_recovers() {
    let dir = tempdir().unwrap();
    let path = dir.path().join("outbox.redb");
    std::fs::write(&path, b"this is not a valid redb database").unwrap();

    let s = ResultStore::open_or_degrade(path.clone());
    assert!(
        s.enabled(),
        "store must recover after quarantining a corrupt db"
    );
    assert!(
        path.with_extension("redb.corrupt").exists(),
        "corrupt file must be quarantined to .redb.corrupt"
    );

    let (key, rec) = record("ex", "a");
    assert!(s
        .journal_batch("ex".to_string(), vec![(key, rec)])
        .await
        .unwrap());
}
