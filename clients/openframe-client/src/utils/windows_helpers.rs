...
/// Human-friendly display name for a tool's Start Menu entry.
/// `openframe-chat` -> `OpenFrame Chat`; otherwise title-case the id on `-`/`_`.
fn friendly_app_name(tool_agent_id: &str) -> String {
    if tool_agent_id == "openframe-chat" {
        return "OpenFrame Chat".to_string();
    }
    tool_agent_id
        .split(['-', '_'])
        .filter(|w| !w.is_empty())
...
/// All-users Start Menu shortcut path for a given display name.
fn start_menu_shortcut_path(display_name: &str) -> PathBuf {
    let program_data =
        std::env::var("ProgramData").unwrap_or_else(|_| r"C:\ProgramData".to_string());
    PathBuf::from(program_data)
        .join(r"Microsoft\Windows\Start Menu\Programs")
        .join(format!("{}.lnk", display_name))
}

/// Escape a value for safe interpolation inside a PowerShell single-quoted string literal.
fn ps_escape_single(s: &str) -> String {
    s.replace('\'', "''")
}

/// Run a short PowerShell script synchronously with no visible window.
fn run_powershell(script: &str) -> Result<()> {
    use std::os::windows::process::CommandExt;
    const CREATE_NO_WINDOW: u32 = 0x08000000;

    let ps_path = crate::platform::get_powershell_path()
        .map_err(|e| anyhow::anyhow!("PowerShell not found: {}", e))?;
...
/// Create an all-users Start Menu shortcut pointing at `target_exe` so the GUI app can be
/// launched manually from Start. The shortcut uses the exe's embedded icon and no extra args,
/// so clicking it opens the app window (not the tray-only `--background` mode used at logon).
pub(crate) fn create_start_menu_shortcut(tool_agent_id: &str, target_exe: &str) -> Result<()> {
    let display_name = friendly_app_name(tool_agent_id);
    let lnk_path = start_menu_shortcut_path(&display_name);
    let lnk_str = lnk_path.to_string_lossy().to_string();
    let working_dir = std::path::Path::new(target_exe)
        .parent()
    ...
    let script = format!(
        "$ErrorActionPreference='Stop'; \
         ...
         $s.Description='{desc}'; \
         $s.Save()",
        lnk = ps_escape_single(&lnk_str),
        exe = ps_escape_single(target_exe),
        dir = ps_escape_single(&working_dir),
        desc = ps_escape_single(&display_name),
    );

    run_powershell(&script)
        .with_context(|| format!("Failed to create Start Menu shortcut at {}", lnk_str))?;
    info!("Created Start Menu shortcut: {} -> {}", lnk_str, target_exe);
    Ok(())
}

/// Remove the all-users Start Menu shortcut for a tool, if present.
pub(crate) fn remove_start_menu_shortcut(tool_agent_id: &str) {
    let display_name = friendly_app_name(tool_agent_id);
    let lnk_path = start_menu_shortcut_path(&display_name);
    if !lnk_path.exists() {
        return;
    ...
    let script = format!(
        ...
        ps_escape_single(&lnk_str)
    );
    match run_powershell(&script) {
        Ok(()) => info!("Removed Start Menu shortcut: {}", lnk_str),
        Err(e) => warn!("Failed to remove Start Menu shortcut {}: {:#}", lnk_str, e),
    }
}
