...
if [ "$REQUEST_METHOD" = "POST" ]; then
  ...
  # Generate authentication string from username and password
  AUTH_STRING=$(echo -n "${MESH_USER}:${MESH_PASS}" | base64)
  
  WS_URL="${MESH_PROTOCOL}://${MESH_NGINX_NAT_HOST}:${MESH_EXTERNAL_PORT}/meshrelay.ashx?id=${DEVICE_ID}&auth=${AUTH_STRING}&p=${PROTOCOL}&rtc=0"
